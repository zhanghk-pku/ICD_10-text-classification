...
class LoadData(object):
    ...
    def data_split(self, text_data, seq_length, hidden_dim, test_rate):
        ...
        for i in test_id:
            test_x.append(x_padding[i])
            test_y.append(y_one_hot[i])
            train_x.remove(x_padding[i])
            train_y.remove(y_one_hot[i])
            mask_test.append(mask_matrix[i])
            mask_train.remove(mask_matrix[i])

        return train_x, train_y, test_x, test_y, mask_train, mask_test
