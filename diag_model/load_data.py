...
class LoadData(object):
    def __init__(self,data_file, filter_num, add_feature):
        ...
        self.feature_names = ['出院诊断']
        self.label_name = ['编码']
        raw_data = pd.read_csv(self.data_file, names=self.feature_names + self.label_name, header=None)
        raw_data = raw_data[['出院诊断','编码']]
        dropna_data = raw_data.dropna(axis=0, how='all', subset=['出院诊断'])
        dropna_data = dropna_data.sample(frac=1).reset_index(drop=True)
        dropna_data.index = range(len(dropna_data))
