    ...
    def lstm_model(self, train_x, train_y, test_x, test_y, keyword_id, mask_train, mask_test,
        ...
        if self.add_keyword_attention:
            ...
            with tf.variable_scope('attention'):
                ...
                print(trans_keyword_emb.shape)
                expand_keyword_emb = tf.expand_dims(tf.transpose(trans_keyword_emb), 0)
                print(first_outputs.shape)
                atten_dot = tf.matmul(first_outputs,tf.tile(expand_keyword_emb, multiples=[batch_size, 1, 1]))
